...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_rgb, state_robot, action_size, BATCH_SIZE, TAU, LEARNING_RATE, critic_full_input, critic_obj_input, full_dim_robot, state_dim_object, action_dim):
        ...
        with tf.variable_scope('Critic', reuse=True):
            self.crit_model, self.action, self.state_full,self.state_obj = self.create_critic_network(full_dim_robot, state_dim_object, action_dim, scope='eval_net', trainable=True)
            self.critic_actor_weights = self.crit_model.get_weights()
                # full_dim_robot, state_dim_object, action_dim, goal_critic_dim
        #Now create the model
        with tf.variable_scope('Actor'):
            self.model , self.weights, self.state_rgb, self.state_rob, self.preact_tensor = self.create_actor_network(state_rgb, state_robot, action_size, scope='eval_net', trainable=True)
            self.target_model, self.target_weights, self.target_state_rgb, self.target_state_rob, __ = self.create_actor_network(state_rgb, state_robot, action_size, scope='target_net', trainable=False)
        ...
        with tf.variable_scope('actor_loss'):     
            ...
            self.actor_loss = tf.multiply(self.isPretrain, -tf.reduce_mean(self.crit_model([self.critic_full_input, self.critic_obj_input, self.joint_vel_scale* self.model([self.state_rgb, self.state_rob])[0]])[0]), name='actor_main_loss')
            # Behaviour cloning
            ...
            with tf.variable_scope('actor_preact_penalty'):  
                ...
                self.actor_penalty_loss = SATUR_PEN_COEF*tf.reduce_mean(tf.norm(self.preact_tensor))            
            ...
            with tf.variable_scope('actor_bottleneck_loss'):
                ...
                # self.actor_bottleneck_loss = tf.reduce_mean(self.env_full_state-self.model([self.state_rgb, self.state_rob])[1]) 
                self.actor_bottleneck_loss = tf.reduce_mean(tf.square(self.env_full_state-self.model([self.state_rgb, self.state_rob])[1]) )
            ...
            with tf.variable_scope('bc_loss'):
                ...
                # self.bc_loss = LAMBDA_BC*tf.reduce_sum(tf.square(self.joint_vel_scale*self.model([self.state_rgb, self.state_rob])[0]-self.sampled_action)*self.isDemo, name='bc_loss_sum') #*tf.cast(self.isDemo,tf.int32)*tf.cast(tf.greater(self.critic_sample_pred, self.critic_target_pred), tf.int32) # x>y # network prediction, sample action
                self.bc_loss = LAMBDA_BC*tf.reduce_mean(tf.square(self.joint_vel_scale*self.model([self.state_rgb, self.state_rob])[0]-self.sampled_action)*self.isDemo, name='bc_loss_sum') #*tf.cast(self.isDemo,tf.int32)*tf.cast(tf.greater(self.critic_sample_pred, self.critic_target_pred), tf.int32) # x>y # network prediction, sample action


            self.actor_total_loss = self.actor_loss + self.bc_loss + self.actor_penalty_loss + self.actor_bottleneck_loss
        ...
        s.append(tf.summary.scalar('Bottleneck_loss', self.actor_bottleneck_loss,family='actor_summary'))
        s.append(tf.summary.scalar('Action_Penalty_loss', self.actor_penalty_loss,family='actor_summary'))
...
# Parameter Space Noise # Parameter Space Noise # Parameter Space Noise # Parameter Space Noise # Parameter Space Noise
    ...
    def _setup_param_noise(self):
        ...
        with tf.variable_scope("Actor/noise", reuse=False): # configure perturbed actor
            self.param_noise_actor , _, _, _, _= self.create_actor_network(self.state_rgb, self.state_rob, self.action_size, scope='policy', trainable=True)
        ...
        with tf.variable_scope("Actor/adapt_noise", reuse=False): # configure separate copy for stddev adoptation 
            self.adaptive_param_noise_actor , _, _, _, _= self.create_actor_network(self.state_rgb, self.state_rob, self.action_size, scope='policy', trainable=True)
        ...
        with tf.variable_scope("noise_update_func", reuse=False):
            ...
            self.perturb_adaptive_policy_ops = self.get_perturbed_actor_updates('Actor/eval_net/', 'Actor/adapt_noise/policy',
                                                                           ...
                                                                           verbose=0)
            self.adaptive_policy_distance = tf.sqrt(tf.reduce_mean(tf.square(self.model([self.state_rgb, self.state_rob])[0] 
                - self.adaptive_param_noise_actor([self.state_rgb, self.state_rob])[0])))


    def _adapt_param_noise(self, state_rgb, state_rob):
        """
...
# Parameter Space Noise # Parameter Space Noise # Parameter Space Noise # Parameter Space Noise # Parameter Space Noise
    ...
    def train_actor_with_flatgrad(self, states_rgb, states_rob, env_full_state, critic_full_input, critic_obj_input, sampled_action, isDemo, isPretrain):
    ...
    '''
    Nature-style CNN architecture

    n_filters=32, filter_size=8, stride=4
    n_filters=64, filter_size=4, stride=2
    n_filters=64, filter_size=3, stride=1
    '''
    def create_actor_network(self, state_rgb, state_robot, action_dim, scope, trainable):
        ## Assymetric Actor ##
        with tf.variable_scope(scope):
            ...
            AH2= Conv2D(64, (4, 4), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_conv2',trainable=trainable)(A1)
            ...
            AH3= Conv2D(64, (3, 3), strides=(1,1), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_conv3',trainable=trainable)(A2)
            ...
            F1 = Flatten()(A3)
            # F1 = Flatten()(AH4)
            BTN_State = Dense(14, kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), activation='linear', name='bottleneck_state', trainable=trainable)(F1)
            # Bottleneck # Bottleneck # Bottleneck

            FC1 = Dense(200, kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), name='actor_fc1',trainable=trainable)(BTN_State)
            LNFC2 = LayerNormalization()(FC1) # Q is scalar value!! dimenstion should be 1
            AFC1 = Activation('relu')(LNFC2)
            
            FC2= Dense(100,kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), name='actor_fc2',trainable=trainable)(AFC1)
            ...
            model = Model(input= [S_rgb,S_robot],output=[Action, BTN_State])
            # model.compile(loss='mse', optimizer=adam)
            # model.compile(loss='mse', optimizer=adam)
            if scope=='eval_net':
                model.summary()
            return model, model.trainable_weights, S_rgb, S_robot, _Action_preact_
    ...
    def create_critic_network(self, state_robot_full, state_obj, action_dim, scope, trainable):
            ## Assymetric Critic ##
            with tf.variable_scope(scope):
                ...
                with tf.name_scope('states'):
                    ...
                    S_obj = Input(shape=[OBJ_STATE], name = 'critic_obj_input') 
                    # G_critic = Input(shape=[CRITIC_GOAL_STATE], name = 'critic_goal_input')
                Concatd = Concatenate(axis=-1)([S_full, S_obj])
                ...
                Q = Dense(1, kernel_initializer=RandomUniform(minval=-3e-3, maxval=3e-3, seed=None),activation='linear', name='q_value',trainable=trainable)(LN3) # Q is scalar value!! dimenstion should be 1
