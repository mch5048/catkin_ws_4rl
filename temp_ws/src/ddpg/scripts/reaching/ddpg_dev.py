...
def playGame(train_indicator=0):    #1 means Train, 0 means simply Run
    ...
    summary_writer = tf.summary.FileWriter(path + 'reaching/summary/ddpg_reacher', sess.graph)
    ...
    try:
        actor.model.load_weights(path+"weights/actormodel_50.h5")
        critic.model.load_weights(path+"weights/criticmodel_50.h5")
        actor.target_model.load_weights(path+"weights/actormodel_50.h5")
        critic.target_model.load_weights(path+"weights/criticmodel_50.h5")
    ...
    for i in range(episode_count): # pseudo: for each episode
        ...
        if i == 0:
            ...
            if not train_indicator:
                s_t0_rms.load_mean_std(path+'mean_std0.txt')
                s_t1_rms.load_mean_std(path+'mean_std1.txt')
                s_t2_rms.load_mean_std(path+'mean_std2.txt')
                s_t3_rms.load_mean_std(path+'mean_std3.txt')
                s_t4_rms.load_mean_std(path+'mean_std4.txt')
