...
class SummaryManager_PPO(object):
    ...
    def setup_train_summary(self): #setpup 
        tls = []

        with tf.variable_scope('train_summary'): # for PPO Actor
            self.ep_ret_ph = tf.placeholder(shape=(), dtype=tf.float32, name='episode_return')
            self.ep_len_ph = tf.placeholder(shape=(), dtype=tf.float32, name='episode_length')
        ...
        with tf.variable_scope('train_summary'):
            tls.append(tf.summary.scalar('episode_return', self.ep_ret_ph))
            tls.append(tf.summary.scalar('episode_length', self.ep_len_ph)) 


        self.epi_summary_op = tf.summary.merge(tls)



    def update_episode_summary(self, epi_ret, epi_len, episode): # inputs are numpy ndarray type
        stat_summary = self.sess.run(self.epi_summary_op, feed_dict={
                                    self.ep_ret_ph: epi_ret, 
                                    self.ep_len_ph: epi_len, })

        self.summary_writer.add_summary(stat_summary, episode)

    def setup_ppo_summary(self): #setpup
        pls = []

        # approx_kl = tf.reduce_mean(logp_old_ph - logp)      # a sample estimate for KL-divergence, easy to compute
        # approx_ent = tf.reduce_mean(-logp)

        with tf.variable_scope('ppo_summary'): # for PPO Actor
            self.kl_ph = tf.placeholder(shape=(), dtype=tf.float32, name='KL_divergence')
            self.ent_ph = tf.placeholder(shape=(), dtype=tf.float32, name='entropy')
            self.cf_ph = tf.placeholder(shape=(), dtype=tf.float32, name='clipfrac')
            self.del_piloss_ph = tf.placeholder(shape=(), dtype=tf.float32, name='delta_pi_loss')
            self.del_vloss_ph = tf.placeholder(shape=(), dtype=tf.float32, name='delta_v_loss')
        ...
        with tf.variable_scope('ppo_summary'):
            pls.append(tf.summary.scalar('KL_divergence', self.kl_ph))
            pls.append(tf.summary.scalar('entropy', self.ent_ph)) 
            pls.append(tf.summary.scalar('clipfrac', self.cf_ph)) 
            pls.append(tf.summary.scalar('delta_pi_loss', self.del_piloss_ph)) 
            pls.append(tf.summary.scalar('delta_v_loss', self.del_vloss_ph)) 

        self.ppo_summary_op = tf.summary.merge(pls)



    def update_ppo_summary(self, KL_d, entropy, clipfrac, delta_pi, delta_v, episode): # inputs are numpy ndarray type
        stat_summary = self.sess.run(self.ppo_summary_op, feed_dict={
                                    self.kl_ph: KL_d, 
                                    self.ent_ph: entropy, 
                                    self.cf_ph: clipfrac, 
                                    self.del_piloss_ph: delta_pi, 
                                    self.del_vloss_ph: delta_v, })
