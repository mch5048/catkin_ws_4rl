...
class PickAndPlace(object):
    ...
    def place(self, pose):
        ...
        # retract to clear object
        self._retract()
ranp=Point(0.4225,0.1265,0.7725)
ranp2=Point(0.4225,-0.1265,0.7725)

def load_gazebo_models(table_pose=Pose(position=Point(x=0.75, y=0.0, z=0.0)),
                       table_reference_frame="world",
                       block_pose=Pose(position=ranp),
                       block_reference_frame="world",
                       block_pose_2=Pose(position=ranp2),
                       block_reference_frame_2="world"):
    ...
    # Load Block URDF
    block_xml_2 = ''
    with open (model_path + "block/BOX1.urdf", "r") as block_file_2:
        block_xml_2=block_file_2.read().replace('\n', '')
    # Spawn Table SDF
    ...
    rospy.wait_for_service('/gazebo/spawn_urdf_model')
    try:
        spawn_urdf_2 = rospy.ServiceProxy('/gazebo/spawn_urdf_model', SpawnModel)
        resp_urdf_2 = spawn_urdf("block_2", block_xml_2, "/",
                               block_pose_2, block_reference_frame_2)
    except rospy.ServiceException, e:
        rospy.logerr("Spawn URDF service call failed: {0}".format(e))
