...
INIT_NOISE_STDDEV = 0.01
DES_NOISE_STDDEV = 0.0002
LOSS_COEF_DICT = {'L2coef': 0.01}
# SATUR_PEN_COEF = 0.001
SATUR_PEN_COEF = 1.0
...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_rgb, state_robot, action_size, goal_rgb, BATCH_SIZE, TAU, LEARNING_RATE, critic_full_input, critic_obj_input, critic_goal_state, full_dim_robot, state_dim_object, action_dim, goal_critic_dim):
        ...
        with tf.variable_scope('Actor'):
            self.model , self.weights, self.state_rgb, self.state_rob, self.goal_rgb, self.preact_tensor = self.create_actor_network(state_rgb, state_robot, action_size, goal_rgb, scope='eval_net', trainable=True)
            self.target_model, self.target_weights, self.target_state_rgb, self.target_state_rob, self.target_goal_rgb, __ = self.create_actor_network(state_rgb, state_robot, action_size, goal_rgb, scope='target_net', trainable=False)
        ...
        with tf.variable_scope('actor_loss'):     
            ...
            self.actor_loss = tf.multiply(self.isPretrain, -tf.reduce_mean(self.crit_model([self.critic_full_input, self.critic_obj_input, self.critic_goal_state, self.joint_vel_scale* self.model([self.state_rgb, self.state_rob, self.goal_rgb])[0]])[0]), name='actor_main_loss')
            # Behaviour cloning
            # tf.where(condition,true state,false state)
            with tf.variable_scope('actor_preact_penalty'):  
                self.actor_penalty_vars = [var for var in self.weights if 'actor_fc2' in var.name] # penalize the preactivation of tanh!          
                # self.actor_penalty_loss = tc.layers.apply_regularization(tc.layers.l2_regularizer(LOSS_COEF_DICT['L2coef']),weights_list=self.actor_penalty_vars)            
                self.actor_penalty_loss = SATUR_PEN_COEF*tf.reduce_sum(tf.square(self.preact_tensor))            

            # Add bottleneck Loss here!# Add bottleneck Loss here! # Add bottleneck Loss here!
            with tf.variable_scope('actor_bottleneck_loss'):
                # actor should infer full state from visual observation
                self.env_full_state = tf.placeholder(tf.float32, shape=[None,14], name='env_full_state')
                self.actor_bottleneck_loss = tf.reduce_mean(self.env_full_state-self.model([self.state_rgb, self.state_rob, self.goal_rgb])[1]) 

                self.goal_env_full_state = tf.placeholder(tf.float32, shape=[None,14], name='env_full_state')
                self.actor_bottlebneck_goal_loss = tf.reduce_mean(self.goal_env_full_state-self.model([self.state_rgb, self.state_rob, self.goal_rgb])[2]) 

                self.actor_bottleneck_total_loss = self.actor_bottleneck_loss + self.actor_bottlebneck_goal_loss
            # Add bottleneck Loss here! # Add bottleneck Loss here! # Add bottleneck Loss here!

            with tf.variable_scope('bc_loss'):
                self.sampled_action = tf.placeholder(tf.float32,[None,action_dim], name='sampled_action_from_batch')        
                self.isDemo = tf.placeholder(tf.float32,[None,action_dim], name='isDemo')
                self.bc_loss = LAMBDA_BC*tf.reduce_sum(tf.square(self.joint_vel_scale*self.model([self.state_rgb, self.state_rob, self.goal_rgb])[0]-self.sampled_action)*self.isDemo, name='bc_loss_sum') #*tf.cast(self.isDemo,tf.int32)*tf.cast(tf.greater(self.critic_sample_pred, self.critic_target_pred), tf.int32) # x>y # network prediction, sample action


            self.actor_total_loss = self.actor_loss + self.bc_loss + self.actor_penalty_loss + self.actor_bottleneck_total_loss
        ...
        with tf.variable_scope('train_actor'):
            ...
            with tf .variable_scope('actor_flatgrad'):
                # self.actor_flatgrad, self.grads = U.flatgrad(self.actor_total_loss, self.weights, clip_norm=5.0)                                                
                self.actor_flatgrad_vars, self.grads_and_vars = self.actor_optimizer.compute_gradients(self.actor_total_loss, self.weights, clip_norm=150.0)
        ...
        s.append(tf.summary.scalar('Bottleneck_loss', self.actor_loss,family='actor_summary'))
    ...
    def _setup_param_noise(self):
        ...
        with tf.variable_scope("noise_update_func", reuse=False):
            ...
            self.perturb_adaptive_policy_ops = self.get_perturbed_actor_updates('Actor/eval_net/', 'Actor/adapt_noise/policy',
                                                                           ...
                                                                           verbose=0)
            self.adaptive_policy_distance = tf.sqrt(tf.reduce_mean(tf.square(self.model([self.state_rgb, self.state_rob, self.goal_rgb])[0] 
                - self.adaptive_param_noise_actor([self.state_rgb, self.state_rob, self.goal_rgb])[0])))
...
# Parameter Space Noise # Parameter Space Noise # Parameter Space Noise # Parameter Space Noise # Parameter Space Noise
    ...
    def train_actor_with_flatgrad(self, states_rgb, states_rob, env_full_state, env_goal_full_state, goal_rgb, critic_full_input, critic_obj_input, critic_goal_state, sampled_action, isDemo, isPretrain):
        ...
        actor_flatgrad, self.train_summary = self.sess.run(train_op, feed_dict={   
            ...
            self.state_rob: states_rob, # now full state is required for training actor
            self.env_full_state: env_full_state[:,:14], # now full state is required for training actor
            self.goal_env_full_state : env_goal_full_state[:,:14],
            self.goal_rgb: goal_rgb,
            self.critic_full_input: critic_full_input,
            self.critic_obj_input: critic_obj_input,
            self.critic_goal_state: critic_goal_state, # use for bottleneck layer
...
# model.add(Lambda(frac_max_pool))
    ...
    def create_actor_network(self, state_rgb, state_robot, action_dim, goal_rgb, scope, trainable):
        ...
        with tf.variable_scope(scope):
            ...
            AH1= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', input_shape=(FRAME_WIDTH, FRAME_HEIGHT, CHANNELS),name='actor_conv1' ,trainable=trainable)(S_rgb)
            # LN1 = Lambda(self.Layer_Normalization, name='layer_norm1')(AH1) # Q is scalar value!! dimenstion should be 1
            BN1 = BatchNormalization()(AH1)
            A1 = Activation('relu')(BN1)
            # A1 = Activation('relu')(AH1)

            AH2= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_conv2',trainable=trainable)(A1)
            # LN2 = Lambda(self.Layer_Normalization, name = "layer_norm2")(AH2) # Q is scalar value!! dimenstion should be 1
            BN2 = BatchNormalization()(AH2)
            
            A2 = Activation('relu')(BN2)
            # A2 = Activation('relu')(AH2)

            AH3= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_conv3',trainable=trainable)(A2)
            # LN3 = Lambda(self.Layer_Normalization, name = "layer_norm3")(AH3) # Q is scalar value!! dimenstion should be 1
            BN3 = BatchNormalization()(AH3)
            
            A3 = Activation('relu')(BN3)

            AH4= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_conv4',trainable=trainable)(A3)
            # LN3 = Lambda(self.Layer_Normalization, name = "layer_norm3")(AH3) # Q is scalar value!! dimenstion should be 1
            # BN4 = BatchNormalization()(AH4)
            
            F1 = Flatten()(AH4)


            ## Implement BottleNeck layer HERE! ## ## Implement BottleNeck layer HERE! ## ## Implement BottleNeck layer HERE! ##
            # env's full state, e.g. joint pos, vels, effes should be added for feed dict # 
            ...
            BTN_State = Dense(14, kernel_initializer=RandomUniform(minval=-3e-3, maxval=3e-3, seed=None), name='bottleneck_state', trainable=trainable)(F1)
            ...
            #################################################################################################################################################
            AGH1= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_goal_conv1',trainable=trainable, input_shape=(FRAME_WIDTH, FRAME_HEIGHT, CHANNELS))(G_rgb)
            # LNG1 = Lambda(self.Layer_Normalization, name = "layer_norm4")(AGH1) # Q is scalar value!! dimenstion should be 1
            BNG1 = BatchNormalization()(AGH1)
            
            A5 = Activation('relu')(BNG1)
            # A4 = Activation('tanh')(AGH1)

            AGH2= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_goal_conv2',trainable=trainable)(A5)
            # LNG2 = Lambda(self.Layer_Normalization, name = "layer_norm5")(AGH2) # Q is scalar value!! dimenstion should be 1
            BNG2 = BatchNormalization()(AGH2)
            
            A6 = Activation('relu')(BNG2)
            # A5 = Activation('tanh')(AGH2)

            AGH3= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_goal_conv3',trainable=trainable)(A6)
            # LNG3 = Lambda(self.Layer_Normalization, name = "layer_norm6")(AGH3) # Q is scalar value!! dimenstion should be 1
            BNG3 = BatchNormalization()(AGH3)
            
            A7 = Activation('relu')(BNG3)

            AGH4= Conv2D(64, (2, 2), strides=(2,2), kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), padding='valid', name='actor_goal_conv4',trainable=trainable)(A7)
            # LNG3 = Lambda(self.Layer_Normalization, name = "layer_norm6")(AGH3) # Q is scalar value!! dimenstion should be 1
            # BNG4 = BatchNormalization()(AGH4)

            FG1 = Flatten()(AGH4)
            BTN_G_State = Dense(14, kernel_initializer=RandomUniform(minval=-3e-3, maxval=3e-3, seed=None), name='bottleneck_goal_state', trainable=trainable)(FG1)

            
            # A8 = Activation('relu')(BNG4)
            ...
            # Concatd = Concatenate(axis=-1)([A4, A8])
            # F2 = Flatten()(A8)

            Concatd = Concatenate(axis=-1)([BTN_State, BTN_G_State])
            # F = Flatten()(Concatd)
            ...
            # THIS LAYER SHOULD BE SUBSTITUTED WITH BOTTLENECK LAYER
            # Concatd2 = Concatenate()([F, S_robot])

            # Concatd2 = Concatenate()([F, S_robot])

            FC1 = Dense(512, kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), name='actor_fc1',trainable=trainable)(Concatd)
            LNFC2 = LayerNormalization()(FC1) # Q is scalar value!! dimenstion should be 1
            ...
            FC2= Dense(512,kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), name='actor_fc2',trainable=trainable)(AFC1)
            LNFC3 = LayerNormalization()(FC2) # Q is scalar value!! dimenstion should be 1
            ...
            # divide for penalty
            _Action_preact_= Dense(action_dim, kernel_initializer=RandomUniform(minval=-3e-3, maxval=3e-3, seed=None),activation=None,name='preactivation',trainable=trainable)(AFC2)
            
            Action = Activation('tanh')(_Action_preact_)         



            # backup of previous layser
            # Action= Dense(action_dim, kernel_initializer=lambda shape:VarianceScaling(scale=1e-4)(shape),activation='tanh')(BFC4)
            # adam = Adam(lr=self.LEARNING_RATE)
           
            model = Model(input= [S_rgb,S_robot, G_rgb],output=[Action, BTN_State, BTN_G_State])
            # model.compile(loss='mse', optimizer=adam)
            # model.compile(loss='mse', optimizer=adam)
            if scope=='eval_net':
                model.summary()
            return model, model.trainable_weights, S_rgb, S_robot, G_rgb, _Action_preact_


            # apply penalization of pre-activation
    ...
    def create_critic_network(self, state_robot_full, state_obj, action_dim, goal_critic_dim, scope, trainable):
        ...
        with tf.variable_scope(scope):
            ...
            A1 = Activation('selu')(LN1)

            Concatd_2 = Concatenate(axis=-1)([A1, A])  

            FC2 = Dense(256, kernel_initializer=keras.initializers.Orthogonal(gain=np.sqrt(2)), name='critic_fc2',  trainable=trainable)(Concatd_2)
            LN2 = Lambda(self.Layer_Normalization, name='layer_norm2')(FC2) # Q is scalar value!! dimenstion should be 1
            A2 = Activation('selu')(LN2)     
    ...
    # Backup network architecture for Actor # Backup network architecture for Actor # Backup network architecture for Actor
    # Backup network architecture # Backup network architecture # Backup network architecture # Backup network architecture
    # Backup network architecture # Backup network architecture # Backup network architecture # Backup network architecture
