...
# mpi Adam adapted form OpenAi Spinninbg rr
...
class MpiAdamOptimizer(tf.train.AdamOptimizer):
    ...
    def __init__(self, learning_rate):
        ...
        tf.train.AdamOptimizer.__init__(self, learning_rate)

    def compute_gradients(self, loss, var_list, clip_norm):
        """
        Same as normal compute_gradients, except average grads over processes.
        """
        grads_and_vars = super(tf.train.AdamOptimizer, self).compute_gradients(loss, var_list)
        if clip_norm is not None:
            grads_and_vars = [(tf.clip_by_norm(grad, clip_norm=clip_norm), v) for grad, v in grads_and_vars]
        ...
        return avg_grads_and_vars, grads_and_vars

    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        """
        Same as normal apply_gradients, except sync params after update.
        """
        opt = super(tf.train.AdamOptimizer, self).apply_gradients(grads_and_vars, global_step, name)
