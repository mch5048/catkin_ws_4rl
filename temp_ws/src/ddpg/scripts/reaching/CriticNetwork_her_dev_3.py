...
class CriticNetwork(object):
    def __init__(self, sess, state_robot_full, state_obj, action_size, goal_critic_dim, BATCH_SIZE, TAU, LEARNING_RATE):
        ...
        with tf.variable_scope('train_critic'):
            ...
            with tf.variable_scope('critic_flatgrad'):
                ...
                self.critic_flatgrad_vars, self.grads_and_vars = self.critic_optimizer.compute_gradients(self.critic_total_loss, self.weights, clip_norm=200.0)
    ...
    def train_critic_with_flatgrad(self, priority_weight, onestep_target, Nstep_target, state_full, state_obj, goal_state_critic, action):
        with tf.variable_scope('update_critic_param'):
            ...
            train_op = [self.train_critic_op, self.critic_summary_op, self.onestep_loss, self.Nstep_loss, self.onestep_td_err, self.Nstep_td_err, self.q_pred, self.critic_reg_loss]
            # train_op = [self.critic_flatgrad, self.critic_summary_op, self.onestep_loss, self.Nstep_loss, self.onestep_td_err, self.Nstep_td_err, self.q_pred]
                            #1                             #2               #3              #4                      #5                  #6              #7             #8
            self.train_out = self.sess.run(train_op, feed_dict={
                self.priority_weight:priority_weight, 
                self.onestep_target: onestep_target,
        ...
        # self.critic_optimizer.apply_gradients(self.train_out[0])
