...
class CriticNetwork(object):
    def __init__(self, sess, state_robot_full, state_obj, action_size, BATCH_SIZE, TAU, LEARNING_RATE):
        ...
        with tf.variable_scope('Critic'):
            self.model, self.action, self.state_full,self.state_obj = self.create_critic_network(state_robot_full, state_obj, action_size, scope='eval_net', trainable=True)
            self.model_2, self.action_2, self.state_full_2,self.state_obj_2 = self.create_critic_network(state_robot_full, state_obj, action_size, scope='eval_net_2', trainable=True)
        ...
        # self.target_model, self.target_action, self.target_state_full, self.target_state_obj  = self.create_critic_network(state_robot_full,  action_size)
            self.target_model, self.target_action, self.target_state_full, self.target_state_obj = self.create_critic_network(state_robot_full, state_obj, action_size, scope='target_net', trainable=False)
            self.target_model_2, self.target_action_2, self.target_state_full_2,self.target_state_obj_2 = self.create_critic_network(state_robot_full, state_obj, action_size, scope='target_net_2', trainable=False)
        ...
        with tf.variable_scope('critic_loss'):  
            ...
            with tf.variable_scope('q1_loss'):
                ...
                self.q_pred_1= self.model([self.state_full, self.state_obj, self.action])   
            ...
            with tf.variable_scope('q2_loss'):
                ...
                self.q_pred_2= self.model_2([self.state_full_2, self.state_obj_2, self.action_2])       
    ...
    # just needed to add 
    def train_critic_with_flatgrad(self, priority_weight, onestep_target, Nstep_target, state_full, state_obj, action):
        with tf.variable_scope('update_critic_param'):
            ...
            self.train_out = self.sess.run(train_op, feed_dict={
                ...
                self.action_2: action,  # feed_dict for critic
                        ...
                        # 1step loss                           ta nstep Loss                             onestep tderr      nstep tderr             q_pred              l2 reg loss                                                  
